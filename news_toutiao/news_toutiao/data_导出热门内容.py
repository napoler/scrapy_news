...
def run():
    ...
    # print(len(data))
    l=len(data)
    train_data=data[:int(l*0.85)]
    dev_data=data[int(l*0.85):]
    add_data(train_data,path='data/hot/',name="train.json")
    add_data(dev_data,path='data/hot/',name="dev.json")
...
def test():
    ...
    """
    i=0
    model = classify(model_name_or_path='data/hot-check/', num_labels=2, device='cuda')
    data=[]
    for it in  DB.article.find({}):
        item={}
